...
export class ListeMessagesComponent {
  ...
  private getTableData(pageOption: pageSelection = { skip: 0, limit: 10 }): void {
    ...
    if (token) {
      ...
      this.http.get('http://localhost:8089/api/messages', { headers }).subscribe(
        (data: any) => {
  ...
  deleteMessage(id: number) {
    ...
    if (token) {
      const headers = new HttpHeaders({ Authorization: 'Bearer ' + token });
      this.http.delete('http://localhost:8089/api/messages/delete/' + id, { headers }).subscribe(
        (data: any) => {
          this.tableData = this.tableData.filter((f: any) => f.id != id);
        },
        error => {
          console.error('Error fetching messages data:', error);
        }
